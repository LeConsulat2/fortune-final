...
interface UseUserMemoryReturn {
  ...
  isLoaded: boolean;
  updatedUserMemory: (updates: Partial<UserMemory>) => void;
...
export function useUserMemory(): useUserMemoryReturn {
  ...
  useEffect(() => {
    try {
      const storedMemory = sessionStorage.get(USER_MEMORY_KEY);
      if (storedMemory) {
...
// check if essentail info is complete (gender, birthDate, name)
...
export function userMemoryToPersonalInfo(memory: UserMemory): PersonalInfo {
  return {
    ...
    zodiacSign:
      memory.zodiacSign || calculateZodiacSign(memory.birthDate || ''),
  };
}
